    ...
    def recursive_schema_query do
      %ObjectType{
        ...
        fields: fn() -> %{
          ...
          b: %{type: TestSchema.recursive_schema_query, resolve: fn(_,_,_,_) -> %{} end },
          c: %{type: TestSchema.recursive_schema_2, resolve: fn(_,_,_,_) -> %{} end }
        } end
      }
    end
        ...
        fields: %{
          person: %{
            type: %ObjectType{
              name: "Person",
              fields: %{
                id:   %{name: "id",   type: %ID{}, resolve: fn(p, _, _, _) -> p.id   end},
                name: %{name: "name", type: %String{}, resolve: fn(p, _, _, _) -> p.name end},
                age:  %{name: "age",  type: %Int{},    resolve: fn(p, _, _, _) -> p.age  end}
              }
            },
            args: %{
              id: %{type: %ID{}}
            },
            resolve: fn(data, %{id: id}, _, _) ->
      ...
      query: %ObjectType{
        ...
        fields: %{
          numbers: %{
            ...
            resolve: fn(_, %{nums: nums}, _, _) -> nums end
          }
        }
      }
    })
      ...
      query: %ObjectType{
        ...
        fields: %{
          person: %{
            ...
            resolve: fn(_, _, _, _) -> %{id: "1", name: "Dave"} end
          }
        }
      }
    })
      ...
      mutation: %ObjectType{
        ...
        fields: %{
          person: %{
            ...
            resolve: fn(_root, args, _, _schema) ->
